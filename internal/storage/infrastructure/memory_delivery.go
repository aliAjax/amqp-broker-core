...
func (m *Memory) ListReady(ctx context.Context, address string, limit int) ([]delivery.Message, error) {
	...
	for _, msg := range m.messages {
		if msg.Address == address && (msg.State == delivery.StateAvailable || msg.State == delivery.StateReleased) {
...
func (m *Memory) Depth(ctx context.Context, address string) (int, error) {
	...
	n := 0
	for _, msg := range m.messages {
		if msg.Address == address && (msg.State == delivery.StateAvailable || msg.State == delivery.StateReleased || (msg.State == delivery.StateAcquired && msg.TransactionID == "")) {
			n++
		}
	}
	return n, nil
